...
#'
#Returns a vecotr that represents a hyperplane that linearly separates the binary response in input space.
#'
...
perceptron_learning_algorithm = function(Xinput, y_binary, MAX_ITER = 1000, w_0 = NULL){
  ...
  for(iter in 1:MAX_ITER){
   for(i in 1:n){
     ...
     w = w_prev+ (y_binary[i] - y_hat_i) * Xinput[i, ]
   }
...
1 or the blue dots are in the third dimension.
...
I have 0s so I dont know where i went wrong....
...
It is not "satisfying" because it's not optimal as it does not cut the wedge in the middle, which ruins the max margin hyperplane.
...
Yes b/c it splits the wedge in the middle.
...
```{r}
lmmod = lm(child ~ parent, data = Galton)
coefs = coef(lmmod)
b_0 = coefs[1]
b_1 = coefs[2]
summary(lmmod)$sigma
...
n = 50
x = runif(n)
y = figure out some function of x of 50% r squared 50 rse
